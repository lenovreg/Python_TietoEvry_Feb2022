...
files = list(files)
print(files)
only_files=[f for f in files if f.is_file()]#make sure we only get files not folders
only_dirs=[f for f in files if f.is_dir()]
print('only_files=', only_files)
print('only_dirs=', only_dirs)

text_files=[f for f in files if f.suffix == ".txt"]
print(f'{text_files=}')
text_files_also=[f for f in Path(".").glob("*.txt") if f.is_file()]
print(f'{text_files_also=}')

#I can use rglob to recursively get all files in a directory and subdirectories
#".." is parent directory
#"**/*.txt" - wildcard for all files
all_text_files_in_parent_dir = [f for f in Path("..").rglob("**/*.csv") if f.is_file()]
print(f'{all_text_files_in_parent_dir=}')
all_text_files_in_parent_dir = [f for f in Path("..").rglob("*.csv") if f.is_file()]
print(f'{all_text_files_in_parent_dir=}')
...
with open('frost.txt') as f:  # create a file object, default is read mode
    ...
    f.seek(0)  # put the needle of the record to beginning - failu lasām atkal no sākuma
...
#         f.writelines("\n".join(filtered_lines))
#         file_out.writelines(filtered_lines) #writelines neliek jaunā rindā
#         f.write("*"*40+"\n")
file_path = Path("frost_cleaned_o29.txt")
with open(file_path, mode="a", encoding="utf-8") as file_out:
    print(f"Writing {len(filtered_lines)} lines into {file_path}")
    file_out.write("\n".join(filtered_lines))
    file_out.writelines(filtered_lines) #writelines neliek jaunā rindā
    file_out.write("*"*40+"\n")
