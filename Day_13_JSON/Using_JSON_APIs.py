...
my_tuple = ("Valdis", 14, "Ābolu ēšana", None, True, False, [1, 2, 3], {"a": 43, "b": 30})
print(my_tuple)
...
with open("data_file_indented_ch_unicode.json", encoding="UTF8") as f:
    my_data_ch = json.load(f)  # if json is malformed then you will get some sort of error
print(my_data_ch)
#
# # In[21]:
...
with open("data_from_url.csv", mode="w") as f:
    ...
    for row in data_from_json:  # each row is a dictionary
        values = ",".join([str(item) for item in row.values()])
        f.write(values + "\n")
